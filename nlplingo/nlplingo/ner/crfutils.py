...
# X is a list of dictionary, where each dictionary contains features (e.g. wordshape, prefix, suffix) about the word
def apply_templates(X, templates):
    ...
    for template in templates:
        # each 'template' is either '((name, i),)' or '((name, i), (name, i+1))'
        # for f, o in template: this will generate either 
        # - [(name, i)]              OR  (where f=name, o=i)
        # - [(name, i), (name, i+1)]
        name = '|'.join(['%s[%d]' % (f, o) for f, o in template])  # name = feature key
