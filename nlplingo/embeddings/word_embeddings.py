...
class DocumentContextualEmbeddings(object):
    @classmethod
    def load_embeddings_into_doc(cls, doc, embedding_filepath):
        """
        :type doc: nlplingo.text.text_theory.Document
        :type embedding_filepath: str   ; we assume this is a npz file
        """
        ...
        if os.path.exists(embedding_filepath):
            ...
            doc_token_map = data['token_map']
            assert len(doc_embeddings) == len(doc_tokens) == len(doc_token_map) == len(doc.sentences)

            for i, sentence in enumerate(doc.sentences):
                assert len(sentence.tokens) == len(doc_tokens[i]) == len(doc_token_map[i])
            ...
            logger.critical("Embedding npz file {} does not exist".format(embedding_filepath))
